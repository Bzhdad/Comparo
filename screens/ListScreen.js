...
import {addCompareOption, removeCompareOption, clearOptions, closeOption} from "../store/Rating/reducer";
...
function ListScreen({ firstItem, secondItem, clearList}) {
  ...
  const [minusButton, setMinusButton] = useState(false);
        ...
        const Names =() =>{
            return (<View><ComparoName firstItem={firstItem} secondItem={secondItem}/></View>);
        }

        const Buttons = ()=>{
            return (
                <AnimatedView style={styles.buttonContainer}>
                    <AddButton onPress={DeleteLine} disabled={minusButton} name={'minus'}
                               buttonStyle={reanimatedSecondButtonStyle}/>
                    <AddButton onPress={AddLine} disabled={false} name={'plus'}
        ...
        useEffect(() => {
            if (clearList) {
                ...
                timerRef.current = setTimeout(() => {dispatch(clearOptions());CloseButtons();}, 1000);
